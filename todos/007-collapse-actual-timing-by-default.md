...
- [ ] Replace the always-open time editor with a collapsed-by-default interaction.
- [ ] Keep the overflow and sleep-protection flow reachable once the editor is opened.
- [ ] Ensure the control label clearly communicates that it edits actual time, not planned time.

## Acceptance Criteria

- [ ] A block does not show the full `Actual Timing` form until the user expands it.
- [ ] Users can still edit start/end times and trigger overrun handling from the collapsed control.
- [ ] The default Today timeline becomes easier to scan at a glance.
