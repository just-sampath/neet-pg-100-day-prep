create or replace function public._apply_schedule_topic_assignment_delta(
  p_user_id uuid,
...
begin
  if jsonb_typeof(p_removed_keys) = 'array' and jsonb_array_length(p_removed_keys) > 0 then
    delete from schedule_topic_assignments as target
    using (
      select value as source_item_id
      from jsonb_array_elements_text(p_removed_keys)
    ) as removed
    where target.user_id = p_user_id
      and target.source_item_id = removed.source_item_id;
  end if;

  if jsonb_typeof(p_changed_rows) = 'array' and jsonb_array_length(p_changed_rows) > 0 then
    with changed as (
      select
        row_number() over (order by row_value.source_item_id) as parking_order,
        row_value.source_item_id
      from jsonb_populate_recordset(null::schedule_topic_assignments, p_changed_rows) as row_value
      where row_value.user_id = p_user_id
    )
    update schedule_topic_assignments as target
    set item_order = (-30000 + changed.parking_order)::smallint
    from changed
    where target.user_id = p_user_id
      and target.source_item_id = changed.source_item_id;

    insert into schedule_topic_assignments
    select *
    from jsonb_populate_recordset(null::schedule_topic_assignments, p_changed_rows) as row_value
    where row_value.user_id = p_user_id
    on conflict (user_id, source_item_id) do update
    set
      ...
      revision_eligible = excluded.revision_eligible,
      recovery_lane = excluded.recovery_lane,
      phase_fence = excluded.phase_fence,
      notes = excluded.notes,
      revision_type = excluded.revision_type,
      reference_label = excluded.reference_label,
      reference_day_number = excluded.reference_day_number,
      ...
      original_day_number = excluded.original_day_number,
      original_block_key = excluded.original_block_key,
...
revoke all on function public._apply_schedule_topic_assignment_delta(uuid, jsonb, jsonb) from public;
...
begin
  ...
  perform public._apply_schedule_topic_assignment_delta(
    p_user_id,
